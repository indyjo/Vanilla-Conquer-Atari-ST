...
{
	uint16_t r = (uint16_t)(r6 >> 3) & 7u;
	uint16_t g = (uint16_t)(g6 >> 3) & 7u;
	uint16_t b = (uint16_t)(b6 >> 3) & 7u;
	return (uint16_t)((r << 8) | (g << 4) | b);
}

void stvq_ste_to_rgb24(uint16_t ste, uint8_t *r, uint8_t *g, uint8_t *b)
{
	uint8_t rn = (uint8_t)((ste >> 8) & 7u);
	uint8_t gn = (uint8_t)((ste >> 4) & 7u);
	uint8_t bn = (uint8_t)(ste & 7u);
	/* 3-bit → 8-bit expand */
	*r = (uint8_t)((rn << 5) | (rn << 2) | (rn >> 1));
	*g = (uint8_t)((gn << 5) | (gn << 2) | (gn >> 1));
	*b = (uint8_t)((bn << 5) | (bn << 2) | (bn >> 1));
}
