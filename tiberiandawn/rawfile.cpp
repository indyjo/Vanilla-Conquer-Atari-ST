...
int RawFileClass::Open(int rights)
{
	...
	*/
	for (;;) {
		...
		*/
		#if defined(POSIX)
			// POSIX: no Hard_Error_Occured needed
		#elif !defined(WIN32)
...
long RawFileClass::Read(void * buffer, long size)
{
	...
	**	is exhausted and bail.
	*/
	while (size > 0) {
		ssize_t result;
		long desired = size < 32000L ? size : 32000L;

		result = read(Handle, buffer, desired);
		if (result == -1) {
			Error(errno, false, Filename);
		} else if (result == 0) {
			break;  // EOF
			...
			bytesread += result;
			size -= result;
			if (result < desired) break;  // No more data
		}
	}
...
long RawFileClass::Write(void const * buffer, long size)
{
	...
	*/
	while (size > 0) {
		...
		result = write(Handle, buffer, desired);
		if (result == -1) {
			Error(errno, false, Filename);
		} else {
			buffer = (char *)buffer + result;
	...
	*/
	if (Is_Open()) {
		...
		*/
		...
		if (lseek(Handle, current_pos, SEEK_SET) == -1) {
			Error(errno, false, Filename);
		}
#else
...
unsigned long RawFileClass::Get_Date_Time(void)
{
	...
	if (RawFileClass::Is_Open()) {
		if (fstat(Handle, &st) == 0) {
			...
			if (tm) {
				unsigned short date = ((tm->tm_year - 80) << 9) | ((tm->tm_mon + 1) << 5) | tm->tm_mday;
				unsigned short time = (tm->tm_hour << 11) | (tm->tm_min << 5) | (tm->tm_sec / 2);
				datetime = ((unsigned long)date << 16) | time;
			}
		}
	} else {
...
bool RawFileClass::Set_Date_Time(unsigned long datetime)
{
	...
	return(false);
#elif defined(POSIX)
	unsigned short date = (unsigned short)(datetime >> 16);
	unsigned short time = (unsigned short)(datetime & 0xFFFF);
	
	// Extract DOS date/time format
	int day = date & 0x1F;
	...
	if (RawFileClass::Is_Open()) {
		// For open files, we need to close, set time, and reopen
		// Save current position first
		off_t current_pos = lseek(Handle, 0, SEEK_CUR);
		int saved_rights = Rights;
		close(Handle);
		int result = utime(Filename, &ut);
		// Reopen with same rights
		if (saved_rights == READ) {
			Handle = open(Filename, O_RDONLY);
		} else if (saved_rights == WRITE) {
			Handle = open(Filename, O_WRONLY);
		} else {
			Handle = open(Filename, O_RDWR);
		}
		if (Handle != -1 && current_pos != -1) {
			lseek(Handle, current_pos, SEEK_SET);
		}
		return (result == 0 && Handle != -1);
	} else {
		if (RawFileClass::Is_Available()) {
			return (utime(Filename, &ut) == 0);
		}
	}
	return false;
#else
