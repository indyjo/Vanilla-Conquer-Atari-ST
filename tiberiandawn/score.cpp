...
void Animate_Cursor(GraphicBufferClass& dest, int pos, int ypos);
...
{
	BlitList.Add(x, y, x, y, String_Pixel_Width((char*)data), 8);
	XPos = x;
	YPos = y;
	Timer.Set(0);
	Timer.Start();
...
void ScorePrintClass::Update(void)
{
	...
	if (Stage && (((char *)DataPtr)[Stage-1]==0) ) {
		...
		BlitList.Add(XPos, YPos, XPos, YPos, (Stage * 6) + 14, 8);
		delete this;
		return;
	}
	...
	if (!Timer.Time()) {
		...
		if (Stage) {
			...
			*/
			...
			TextPrintBuffer->Print(localstr, pos - 6, YPos - 1, TBLACK, TBLACK);
			TextPrintBuffer->Print(localstr, pos - 6, YPos + 1, TBLACK, TBLACK);
			TextPrintBuffer->Print(localstr, pos - 5, YPos, TBLACK, TBLACK);

			Set_Font_Palette(PrimaryPalette);
			TextPrintBuffer->Print(localstr, pos - 6, YPos, TBLACK, TBLACK);
		}
		if (((char *)DataPtr)[Stage]) {
			localstr[0]=((char *)DataPtr)[Stage];
			Set_Font_Palette(_whitepal);
			TextPrintBuffer->Print(localstr, pos, YPos - 1, TBLACK, TBLACK);
			TextPrintBuffer->Print(localstr, pos, YPos + 1, TBLACK, TBLACK);
			TextPrintBuffer->Print(localstr, pos + 1, YPos, TBLACK, TBLACK);
		}
		Stage++;
	}
}
...
void ScoreScaleClass::Update(void)
{
	...
	*/
	if (!Timer.Time()) {
		Timer.Set(1);
		if (Stage != 5) {
			TextPrintBuffer->Blit(*PseudoSeenBuff, _destx[Stage + 1], YPos,
				_destx[Stage + 1], YPos,
				_destw[Stage + 1], _destw[Stage + 1]);
		...
		if (Stage) {
			...
			TextPrintBuffer->Fill_Rect(0, 0, 7, 7, TBLACK);
			TextPrintBuffer->Print((char *)DataPtr, 0,0,   TBLACK, TBLACK);
			TextPrintBuffer->Scale(*PseudoSeenBuff, 0, 0, _destx[Stage], YPos,
				5, 5, _destw[Stage], _destw[Stage], true);
			...
			TextPrintBuffer->Print((char *)DataPtr, XPos, YPos, TBLACK, TBLACK);
...
void TextBlitClass::Update(GraphicViewPortClass& dest)
{
	if (TextPrintBuffer){
		...
		if (dest.Lock()){
			...
			for (int i=0 ; i<Count ; i++){
				TextPrintBuffer->Blit (dest, BlitListo[i].SourceX,
													BlitListo[i].SourceY,
													BlitListo[i].DestX,
													BlitListo[i].DestY,
													BlitListo[i].Width,
...
/* --- Now display the background animation --- */
	...
	*/
	...
	BlitList.Add(264, 26, 264, 26, 24, 6);
	BlitList.Add(264, 38, 264, 38, 24, 6);
	BlitList.Add(264, 50, 264, 50, 24, 6);
	BlitList.Add(275, 9, 275, 9, 32, 6);	// Minutes
	for (i = 0; i <= 160; i++) {
...
void ScoreClass::Do_Nod_Buildings_Graph(void)
{
	...
	*/
	...
	Set_Logic_Page(SysMemPage);
	Call_Back_Delay(30);
	BlitList.Add(BUILDING_X + 8, BUILDING_Y, BUILDING_X + 8, BUILDING_Y, 30, 6);
	BlitList.Add(BUILDING_X + 8, BUILDING_Y + 12, BUILDING_X + 8, BUILDING_Y + 12, 30, 6);
	BlitList.Add(BUILDING_X + 8, BUILDING_Y + 24, BUILDING_X + 8, BUILDING_Y + 24, 30, 6);

	TextPrintBuffer->Print(0, BUILDING_X + 8, BUILDING_Y, TBLACK, TBLACK);
	TextPrintBuffer->Print(0, BUILDING_X + 8, BUILDING_Y + 12, TBLACK, TBLACK);
	TextPrintBuffer->Print(0, BUILDING_X + 8, BUILDING_Y + 24, TBLACK, TBLACK);
	...
	*/
	...
	for (i=0; i<98; i++) {
		...
		*/
		...
		if (!Keyboard->Check()) Call_Back_Delay(1);
	}
...
void ScoreClass::Do_GDI_Graph(void const * yellowptr, void const * redptr, int gkilled, int nkilled, int ypos)
{
	...
	BlitList.Add(297, ypos + 2, 297, ypos + 2, 30, 6);
	...
	if (!Keyboard->Check()) Call_Back_Delay(40);

	BlitList.Add(297, ypos + 14, 297, ypos + 14, 30, 6);
...
void ScoreClass::Do_Nod_Casualties_Graph(void)
{
	...
	*/
	...
	BlitList.Add(SCORETEXT_X + 64, CASUALTY_Y + 2, SCORETEXT_X + 64, CASUALTY_Y + 2, 30, 6);
	BlitList.Add(SCORETEXT_X + 64, CASUALTY_Y + 14, SCORETEXT_X + 64, CASUALTY_Y + 14, 30, 6);
	BlitList.Add(SCORETEXT_X + 64, CASUALTY_Y + 26, SCORETEXT_X + 64, CASUALTY_Y + 26, 30, 6);
	...
	for (i = 1; i <= max; i++) {
		...
		for (int q = 0; q < 3; q++) {
			...
			if (!Keyboard->Check()) Call_Back_Delay(3);
		}
		Play_Sample(Beepy6, 255, Options.Normalize_Sound(110));
	}
	if (Keyboard->Check()) Keyboard->Clear();
...
void ScoreClass::Show_Credits(int house, char const pal[])
{
	...
	*/
	...
	BlitList.Add(_credpx[house], _credpy[house], _credpx[house], _credpy[house], 30, 6);
...
void ScoreClass::Print_Minutes(int minutes)
{
	...
	TextPrintBuffer->Print(str, 275, 9, TBLACK, TBLACK);
}
...
//	HidPage.Blit(SeenBuff, 0, 0, xpos, ypos, width, 8);
	...
	TextPrintBuffer->Fill_Rect(xpos, ypos, xpos + width, ypos + 7, BLACK);
	TextPrintBuffer->Print(destbuf, xpos, ypos, WHITE, TBLACK);
...
void Animate_Cursor(GraphicBufferClass& dest, int pos, int ypos)
{
	...
	if (pos != _lastpos) {
		dest.Draw_Line(HALLFAME_X + (_lastpos*6),ypos, HALLFAME_X + (_lastpos*6) + 5, ypos, TBLACK);
		// TextPrintBuffer->Fill_Rect(HALLFAME_X + (_lastpos * 6), ypos,
		// 	HALLFAME_X + (_lastpos * 6) + 5, ypos + 1, BLACK);
		_lastpos = pos;
		_state = 0;
	}

	dest.Draw_Line(HALLFAME_X + (pos*6),ypos, HALLFAME_X + (pos*6)+5, ypos, _state ? LTBLUE : TBLACK);
	// TextPrintBuffer->Fill_Rect(HALLFAME_X + (pos * 6), ypos,
	// 	HALLFAME_X + (pos * 6) + 5, ypos + 1, _state ? LTBLUE : BLACK);
