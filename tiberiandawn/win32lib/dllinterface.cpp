...
void DLLExportClass::DLL_Draw_Intercept(int shape_number, int x, int y, int width, int height, int flags, ObjectClass *object, const char *shape_file_name, char override_owner, int scale)
{
	...
	HouseClass* owner_house = NULL;
	for (int i = 0; i < Houses.Count(); ++i) {
		HouseClass* hptr = Houses.Ptr(i);
		if ((hptr != NULL) && (hptr->Class->House == new_object.Owner)) {
			owner_house = hptr;
			break;
		}
	}

	new_object.RemapColor = (owner_house != NULL) ? owner_house->RemapColor : -1;
		...
		for (int index = 0; index < Map.Layer[layer].Count(); index++) {
			...
			if (object->IsActive) {
				...
				if (object->Is_Techno()) {
					...
					*/
					TechnoClass* techno_object = static_cast<TechnoClass*>(object);
					TechnoClass* contact_object = techno_object->In_Radio_Contact() ? techno_object->Contact_With_Whom() : NULL;
					if ((contact_object != NULL) && (contact_object->What_Am_I() == RTTI_BUILDING) && contact_object->IsTethered && *((BuildingClass*)contact_object) == STRUCT_WEAP) {
						continue;
					}
				}
...
bool DLLExportClass::Get_Player_Info_State(uint64 player_id, unsigned char *buffer_in, unsigned int buffer_size)
{
	...
	if (CurrentObject.Count() > 0) {
		...
		ObjectClass* action_object = NULL;
