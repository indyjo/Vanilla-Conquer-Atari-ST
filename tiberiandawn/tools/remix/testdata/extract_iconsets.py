#!/usr/bin/env python3
"""
Extract curated terrain iconset blobs from an unmodified TEMPERAT.MIX.
...
# CRC of upper-case filename (Westwood MIX index key).
ICONSETS = (
    "P04.TEM",
)
...
def main() -> int:
    if len(sys.argv) != 2:
        print(f"usage: {sys.argv[0]} TEMPERAT.MIX", file=sys.stderr)
        return 2

    mix_path = Path(sys.argv[1])
    out_dir = Path(__file__).resolve().parent / "iconsets"
