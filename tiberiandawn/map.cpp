...
#endif
{
	...
	*/
#ifdef POSIX
	snprintf(fname, sizeof(fname), "%s.BIN", root);
#else
	...
	*/
	...
	for (i = 0; i < MAP_CELL_TOTAL; i++) {
		unsigned char raw[2];
		if (file.Read(raw, sizeof(raw)) != sizeof(raw)) break;
		TemplateType temp_ttype = (TemplateType)raw[0];
		unsigned char temp_ticon = raw[1];
		if (temp_ttype == (TemplateType)255) {
			temp_ttype = TEMPLATE_NONE;
		}
		if (temp_ttype != TEMPLATE_NONE &&
			((int)temp_ttype < (int)TEMPLATE_FIRST || (int)temp_ttype >= (int)TEMPLATE_COUNT)) {
			temp_ttype = TEMPLATE_NONE;
			temp_ticon = 0;
		}
		...
		*/
		if (temp_ttype != TEMPLATE_CLEAR1 && temp_ttype != TEMPLATE_NONE) {
			TemplateTypeClass const &ttype = TemplateTypeClass::As_Reference(temp_ttype);
			shape = ttype.Get_Image_Data();
			if (shape) {
				rawmap = Get_Icon_Set_Map(shape);
				if (rawmap) {
					map = (char*)rawmap;
					if (temp_ticon >= (ttype.Width * ttype.Height)) {
						temp_ticon = 0;
						temp_ttype = TEMPLATE_NONE;
					} else if (map[temp_ticon] == -1) {
						temp_ticon = 0;
						temp_ttype = TEMPLATE_NONE;
					}
				} else {
		...
		cellptr->TType = temp_ttype;
		cellptr->TIcon = temp_ticon;
		/*
		**	BIN map data only carries template/icon; force overlays to a known state
		**	before recalculating land attributes.
		*/
...
bool MapClass::Read_Binary_File(char const * fname, uint32_t *crc)
{
	...
	*/
	...
	for (i = 0; i < MAP_CELL_TOTAL; i++) {
		...
		*/
		if (temp_ttype != TEMPLATE_CLEAR1 && temp_ttype != TEMPLATE_NONE) {
			shape = TemplateTypeClass::As_Reference(temp_ttype).Get_Image_Data();
			if (shape) {
				rawmap = Get_Icon_Set_Map(shape);
				if (rawmap) {
					map = (char*)rawmap;
