...
`Mark(MARK_CHANGE)` flags cells via subclass `Refresh_Cells` (occupy/overlap). `Redraw_Objects` is **non-contagious**: it sets `CellRedraw` only, and does not `Mark` occupiers. Overlap lists still flag spilled cells for restamp; per-cell overlapper pointer slots are gone (objects come from `Layer[]`).
...
Objects come from `DisplayClass::Layer[]` in existing draw order. Binning is in **lepton space**: each object supplies a conservative map-absolute `Get_AABB` (units/buildings from `Coord`, attached anims from `Center_Coord()`, bullets include altitude). Selection/anim/altitude may enlarge it. Each redraw rect is converted once to a half-open map-lepton box. The bin loop first rejects objects that miss the **union** of those boxes (typical case: one dirty rect, many off-screen or far units). A single rect is that union, so a hit is binned with no inner walk. Several rects use view-cell bitmasks: AABB overlap with a cell-aligned rect is true iff the object's X cells and Y cells both intersect the rect (`CELL_LEPTON_*` is 256, so lepton→cell is `>> 8`). No occupy/overlap walks and no `Coord_To_Pixel` in the bin loop. An object that overlaps two rects is drawn in both (each clipped to that rect). `WINDOW_TACTICAL` matches the redraw rect. Objects in the rect are drawn even if `Tactical_Cell_Hides_Objects_For_Local_Player` is true; the shroud pass after objects covers black/fog cells. Terrain stamps still skip hidden cells (black mask only).
...
After paint, CCR **presents** HidPage to SeenBuff by copying those redraw rects (plus tab/sidebar/power/radar/gadget/credits regions snapshotted from existing `IsToRedraw` flags, the previous and current mouse cursor boxes, and the help tooltip box **once** when it first appears). Later tile restamps under the tooltip already include it. The tooltip cache blits only the real text box(es), not the unused L-corner beside a shorter cost line. Present boxes are grown to 16-pixel X bounds so interleaved bitplanes are a contiguous word stream: one source-only blitter pass (`ST_Blit_Planar_Aligned_Rect_Copy`, hop=source, op=D=S). A **full** 320×200 present (`ST_Blit_Linear_Copy`) is used when the map scrolled this frame (`Scroll_Map` or a pending `DesiredTacticalCoord` change, including `record.bin` playback), after `Flag_To_Redraw(true)`, or when the dirty union is large. Playback does not run `Map.Input` / `Scroll_Map`; it teleports the view, so present cannot rely on mouse-cursor rects.
