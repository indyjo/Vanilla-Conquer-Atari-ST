...
void CellClass::Recalc_Attributes(void)
{
	...
	if (Overlay != OVERLAY_NONE &&
		...
		char dbg[128];
		sprintf(dbg, "[Cell::Recalc_Attributes] invalid Overlay=%d cell=%d",
			(int)Overlay, (int)Cell_Number());
	...
	if (TType != TEMPLATE_NONE &&
		...
		char dbg[128];
		sprintf(dbg, "[Cell::Recalc_Attributes] invalid TType=%d cell=%d",
			(int)TType, (int)Cell_Number());
		GlyphX_Debug_Print(dbg);
		TType = TEMPLATE_NONE;
		TIcon = 0;
	}
...
static bool Resolve_House_Pointer(HouseClass *player, HousesType &out_house)
{
	if (!player) {
		return false;
	}
	for (HousesType h = HOUSE_FIRST; h < HOUSE_COUNT; h++) {
		if (HouseClass::As_Pointer(h) == player) {
			out_house = h;
			return true;
		}
	}
...
{
	HousesType house;
	if (Resolve_House_Pointer(player, house)) {
		Set_Mapped(house, set);
		if (GameToPlay == GAME_NORMAL && PlayerPtr && PlayerPtr->Class &&
