...
#ifdef ATARI_ST
	/*
	** Same idea as SCORE.CPP: alias the compositing buffer (planar). Visible SeenBuff for now;
	** switch to HiddenPage later for true off-screen compositing.
	*/
...
//	SeenBuff.Blit(HidPage);
...
#ifdef ATARI_ST
	Install_Animation_C2P_WeightSet(greyearth);
	...
	for (i = 1; i < Get_Animation_Frame_Count(greyearth); i++) {
		Call_Back_Delay(4);
#ifdef ATARI_ST
		Animate_Frame(greyearth, SysMemPage, i);
		SysMemPage.Blit(*PseudoSeenBuff);
#else
...
#ifdef ATARI_ST
		Animate_Frame(greyearth2, SysMemPage, i);
		SysMemPage.Blit(*PseudoSeenBuff);
#else
...
#ifdef ATARI_ST
	Install_Animation_C2P_WeightSet(progress);
...
//	Set_Font(oldfont);
...
#ifdef ATARI_ST
		Animate_Frame(progress, SysMemPage, startframe + frame);
		SysMemPage.Blit(*PseudoSeenBuff);
#else
	...
	*/
	...
	if (house == HOUSE_GOOD) {
		...
		CCFileClass click_file(lastscenario ? "CLICK_SA.CPS" : "CLICK_A.CPS");
		Load_Uncompress(click_file, SysMemPage, SysMemPage, NULL);
...
// execute delay every other line
		...
		dest->Unlock();
		//if (seen){
			//Interpolate_2X_Scale(dest , seen ,NULL, Settings.Video.InterpolationMode);
		//}
	}
}
