...
class FixedHeapClass
{
	public:
		...
		virtual int Set_Heap(int count);
	...
	protected:
		static inline int Slot_Size(int size)
		{
			return((size + (int)sizeof(unsigned short) + 1) & ~1);
		};
		...
		*/
		...
		/*
		**	This is the stride of each sub-block range within the buffer. This
		**	includes the ID word that precedes the object plus any padding.
		*/
		int Size;
		...
		*/
		...
		/*
		**	Pointer to the first object's memory within the heap buffer.
		*/
		void * Buffer;
...
{
	public:
		TFixedHeapClass(void) : FixedHeapClass(FixedHeapClass::Slot_Size(sizeof(T))) {};
...
{
	public:
		TFixedIHeapClass(void) : FixedIHeapClass(FixedHeapClass::Slot_Size(sizeof(T))) {};
