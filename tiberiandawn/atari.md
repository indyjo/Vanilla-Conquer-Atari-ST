...
Objects come from `DisplayClass::Layer[]` in existing draw order. Binning is in **lepton space**: each object supplies a conservative map-absolute `Get_AABB` (units/buildings from `Coord`, attached anims from `Center_Coord()`, bullets include altitude). Selection/anim/altitude may enlarge it. Each redraw rect is converted once to a half-open map-lepton box. Objects that miss the **union** of those boxes are skipped (typical: one dirty rect vs a full ground layer). Rects are sorted by ascending `x_min`; the per-object loop tests `x_min`, then Y, then `x_max` and stops when `obj.x_max < rect.x_min`. No occupy/overlap walks and no `Coord_To_Pixel` in the bin loop. An object that overlaps two rects is drawn in both (each clipped to that rect). `WINDOW_TACTICAL` matches the redraw rect. Objects in the rect are drawn even if `Tactical_Cell_Hides_Objects_For_Local_Player` is true; the shroud pass after objects covers black/fog cells. Terrain stamps still skip hidden cells (black mask only).
