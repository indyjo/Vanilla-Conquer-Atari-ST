...
`Alt+C` toggles `Debug_Coalesced_Clipped_Redraw` (on by default). `Mark(MARK_CHANGE)` still flags cells via subclass `Refresh_Cells` (occupy/overlap). `Redraw_Objects` stays **non-contagious**: it sets `CellRedraw` only, and does not `Mark` occupiers/overlappers.
