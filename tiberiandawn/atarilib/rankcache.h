 ...
 *
 * get / retarget_oldest return a pointer into the live node so callers can
 * update Value fields (crop meta, etc.) in place.
 */
...
public:
	 ...
	 * value_out points at the live node Value after any promote.
	 */
	 ...
	 * Returns false if empty or if key is already present.
	 */
	bool retarget_oldest(const Key &key, Value *&value_out)
	{
		...
		for (uint16_t logical = 0; logical < capacity_; ++logical) {
			if (nodes_[phys(logical)].key == key)
				return false;
		}
